-- Based on dotnet/orleans v10.3.1 SQLServer-Streaming.sql.
-- FabrCore adaptations: orlns schema, idempotent creation, procedure and query upgrades.
-- Apply with SQLServer-Main.sql first. Does not drop queues or reset message IDs.
/*
...
*/
IF OBJECT_ID(N'orlns.OrleansStreamMessageSequence', 'SO') IS NULL
...
CREATE OR ALTER PROCEDURE orlns.QueueStreamMessage
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'QueueStreamMessageKey' AS QueryKey, 'EXECUTE orlns.QueueStreamMessage @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @Payload = @Payload, @ExpiryTimeout = @ExpiryTimeout' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
...
CREATE OR ALTER PROCEDURE orlns.GetStreamMessages
	@ServiceId NVARCHAR(150),
    @ProviderId NVARCHAR(150),
	@QueueId NVARCHAR(150),
    @MaxCount INT,
...
(
	SELECT TOP (@MaxCount)
		...
		Payload
	FROM
		orlns.OrleansStreamMessage WITH (UPDLOCK, READPAST, READCOMMITTEDLOCK)
	WHERE
		ServiceId = @ServiceId
        AND ProviderId = @ProviderId
		AND QueueId = @QueueId
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'GetStreamMessagesKey' AS QueryKey, 'EXECUTE orlns.GetStreamMessages @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @MaxCount = @MaxCount, @MaxAttempts = @MaxAttempts, @VisibilityTimeout = @VisibilityTimeout, @RemovalTimeout = @RemovalTimeout, @EvictionInterval = @EvictionInterval, @EvictionBatchSize = @EvictionBatchSize' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
WHEN NOT MATCHED THEN INSERT (QueryKey, QueryText) VALUES (source.QueryKey, source.QueryText);
GO
...
CREATE OR ALTER PROCEDURE orlns.ConfirmStreamMessages
	@ServiceId NVARCHAR(150),
    @ProviderId NVARCHAR(150),
	@QueueId NVARCHAR(150),
    @Items NVARCHAR(MAX)
...
BEGIN
    ...
    (
        SELECT TOP (@Count)
            M.*
        FROM
            orlns.OrleansStreamMessage AS M WITH (UPDLOCK, READPAST, READCOMMITTEDLOCK, ROWLOCK)
        WHERE
            ServiceId = @ServiceId
            AND ProviderId = @ProviderId
            AND QueueId = @QueueId
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'ConfirmStreamMessagesKey' AS QueryKey, 'EXECUTE orlns.ConfirmStreamMessages @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @Items = @Items' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
WHEN NOT MATCHED THEN INSERT (QueryKey, QueryText) VALUES (source.QueryKey, source.QueryText);
GO
...
CREATE OR ALTER PROCEDURE orlns.FailStreamMessage
	@ServiceId NVARCHAR(150),
    @ProviderId NVARCHAR(150),
	@QueueId NVARCHAR(150),
    @MessageId BIGINT,
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'FailStreamMessageKey' AS QueryKey, 'EXECUTE orlns.FailStreamMessage @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @MessageId = @MessageId, @MaxAttempts = @MaxAttempts, @RemovalTimeout = @RemovalTimeout' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
WHEN NOT MATCHED THEN INSERT (QueryKey, QueryText) VALUES (source.QueryKey, source.QueryText);
GO
...
CREATE OR ALTER PROCEDURE orlns.EvictStreamMessages
	@ServiceId NVARCHAR(150),
    @ProviderId NVARCHAR(150),
	@QueueId NVARCHAR(150),
	@BatchSize INT,
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'EvictStreamMessagesKey' AS QueryKey, 'EXECUTE orlns.EvictStreamMessages @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @BatchSize = @BatchSize, @MaxAttempts = @MaxAttempts, @RemovalTimeout = @RemovalTimeout' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
WHEN NOT MATCHED THEN INSERT (QueryKey, QueryText) VALUES (source.QueryKey, source.QueryText);
GO
...
CREATE OR ALTER PROCEDURE orlns.EvictStreamDeadLetters
	@ServiceId NVARCHAR(150),
    @ProviderId NVARCHAR(150),
	@QueueId NVARCHAR(150),
	@BatchSize INT
...
MERGE orlns.OrleansQuery WITH (HOLDLOCK) AS target
USING (SELECT 'EvictStreamDeadLettersKey' AS QueryKey, 'EXECUTE orlns.EvictStreamDeadLetters @ServiceId = @ServiceId, @ProviderId = @ProviderId, @QueueId = @QueueId, @BatchSize = @BatchSize' AS QueryText) AS source
ON target.QueryKey = source.QueryKey
WHEN MATCHED THEN UPDATE SET QueryText = source.QueryText
WHEN NOT MATCHED THEN INSERT (QueryKey, QueryText) VALUES (source.QueryKey, source.QueryText);
GO
