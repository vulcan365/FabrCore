...
/// backed by SQL Server, with memory streams by default or optional durable ADO.NET streams.
...
public sealed class SqlServerOrleansProvider : IFabrCoreOrleansProvider
{
    ...
    public void ConfigureSilo(ISiloBuilder siloBuilder, OrleansClusterOptions options, IConfiguration configuration, ILogger logger)
    {
        ...
        switch (options.SqlServerStreams)
        {
            case SqlServerStreamMode.Memory:
                siloBuilder.AddMemoryStreams(FabrCoreOrleansConstants.StreamProviderName);
                break;
            case SqlServerStreamMode.AdoNet:
                siloBuilder.AddAdoNetStreams(FabrCoreOrleansConstants.StreamProviderName, streams =>
                {
                    streams.Invariant = AdoNetInvariant;
                    streams.ConnectionString = options.ConnectionString;
                });
                break;
            default:
                throw new InvalidOperationException($"Unknown SQL Server stream mode: {options.SqlServerStreams}.");
        }
        logger.LogInformation("Orleans SQL Server stream mode: {StreamMode}", options.SqlServerStreams);
    }
}
