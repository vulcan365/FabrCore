using Fabr.Client.Configuration;
using Fabr.Client.Services;
...
{
    public static class FabrClientExtensions
    {
        private static readonly ActivitySource ActivitySource = new("Fabr.Client.Extensions");
        private static readonly Meter Meter = new("Fabr.Client.Extensions");

        // Metrics
        private static readonly Counter<long> ClientsConfiguredCounter = Meter.CreateCounter<long>(
            "fabr.client.extension.configured",
            description: "Number of Fabr clients configured");

        private static readonly Counter<long> ErrorCounter = Meter.CreateCounter<long>(
            "fabr.client.extension.errors",
        ...
        private static readonly Counter<long> ConnectionRetryCounter = Meter.CreateCounter<long>(
            "fabr.client.connection.retries",
            description: "Number of connection retry attempts");

        public static IHostApplicationBuilder AddFabrClient(this IHostApplicationBuilder builder)
        {
            using var activity = ActivitySource.StartActivity("AddFabrClient", ActivityKind.Internal);
            ...
            var logger = loggerFactory.CreateLogger("Fabr.Client.Extensions");

            logger.LogInformation("Adding Fabr client - Environment: {Environment}", builder.Environment.EnvironmentName);
            ...
            try
            {
                ...
                if (clusterClientDescriptor == null)
                {
                    ...
                    builder.UseOrleansClient(client =>
                    {
                        ...
                        client.UseConnectionRetryFilter<FabrClientConnectionRetryFilter>();
                    });
                }
                else
                {
                ...
                // Register FabrHostApiClient with HttpClient
                builder.Services.AddHttpClient<IFabrHostApiClient, FabrHostApiClient>();
                logger.LogInformation("FabrHostApiClient registered with HttpClient");
                ...
                logger.LogInformation("Fabr client added successfully");
            ...
            {
                logger.LogError(ex, "Error adding Fabr client");
        ...
        public static IHost UseFabrClient(this IHost app)
        {
            using var activity = ActivitySource.StartActivity("UseFabrClient", ActivityKind.Internal);
            var logger = app.Services.GetRequiredService<ILogger<IHost>>();

            logger.LogInformation("Configuring Fabr client");

            try
            {
                // Any post-configuration can go here
                logger.LogInformation("Fabr client configured successfully");
                activity?.SetStatus(ActivityStatusCode.Ok);
                return app;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Error configuring Fabr client");
                activity?.SetStatus(ActivityStatusCode.Error, ex.Message);
                activity?.AddException(ex);
        ...
        /// <summary>
        /// Adds Fabr client Blazor components support including the ChatDockManager.
        ...
        /// <code>
        /// // In Program.cs
        /// builder.Services.AddFabrClientComponents();
        ///
        ...
        /// </code>
        /// </remarks>
        public static IServiceCollection AddFabrClientComponents(this IServiceCollection services)
        {
    ...
    /// </summary>
    public sealed class FabrClientConnectionRetryFilter : IClientConnectionRetryFilter
    {
        private readonly OrleansClusterOptions _options;
        private readonly ILogger<FabrClientConnectionRetryFilter> _logger;
        private static readonly ActivitySource ActivitySource = new("Fabr.Client.Connection");
        private static readonly Meter Meter = new("Fabr.Client.Connection");

        private static readonly Counter<long> RetryAttemptCounter = Meter.CreateCounter<long>(
            "fabr.client.connection.retry_attempts",
        ...
        private static readonly Counter<long> ConnectionSuccessCounter = Meter.CreateCounter<long>(
            "fabr.client.connection.success",
        ...
        private static readonly Counter<long> ConnectionFailureCounter = Meter.CreateCounter<long>(
            "fabr.client.connection.failures",
            description: "Number of connection failures after all retries exhausted");

        private int _attemptCount = 0;

        public FabrClientConnectionRetryFilter(
            IConfiguration configuration,
            ILogger<FabrClientConnectionRetryFilter> logger)
        {
            ...
            _logger.LogInformation(
                "Fabr client connection retry filter initialized. MaxRetries: {MaxRetries}, RetryDelay: {RetryDelay}",
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (_attemptCount >= maxAttempts)
            {
                _logger.LogError(
                    ...
                    "Ensure the Fabr server is running and accessible.",
                    maxAttempts,
