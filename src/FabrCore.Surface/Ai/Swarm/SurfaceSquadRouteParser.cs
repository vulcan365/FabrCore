...
public static class SurfaceSquadRouteParser
{
    public static SurfaceSquadRouteResult Resolve(SurfaceSquad squad, string text)
    {
        ...
        if (squad.SquadType == SurfaceSquadType.Task
            && !string.Equals(mention, "planner", StringComparison.OrdinalIgnoreCase))
        {
            return new SurfaceSquadRouteResult(true, squad.OrchestratorHandle, trimmed, null, null);
        }
        ...
        if (string.Equals(mention, "planner", StringComparison.OrdinalIgnoreCase))
        {
            if (squad.SquadType != SurfaceSquadType.Swarm)
            {
                return new SurfaceSquadRouteResult(
                    ...
                    body,
                    mention,
                    $"No planner is available in this {squad.SquadType.ToString().ToLowerInvariant()} squad.");
            }

            return new SurfaceSquadRouteResult(true, squad.PlannerHandle, body, mention, null);
        }
