...
public sealed class SurfaceBasicSquadService : ISurfaceBasicSquadService
{
        ...
        string runtimeJson)
    {
        if (definition.SquadType == SurfaceSquadType.Swarm)
        {
            yield return new AgentConfiguration
            {
                ...
                Models = BlankToDefault(definition.PlannerModel),
                Description = $"Planner for squad {squad.Name}",
        ...
        yield return new AgentConfiguration
        {
            ...
            AgentType = definition.SquadType switch
            {
                SurfaceSquadType.Orchestrator => SurfaceOrchestrationAgentTypes.SquadOrchestrator,
                SurfaceSquadType.Task => SurfaceSwarmAgentTypes.TaskRunner,
                _ => SurfaceSwarmAgentTypes.Orchestrator
            },
        ...
        SurfaceSquadDefinition definition)
    {
        ...
        var plannerAlias = SurfaceSquadHandleBuilder.BuildPlannerAlias(slug);
        return new SurfaceSquad
        {
            ...
            PlannerHandle = SurfaceSquadHandleBuilder.Qualify(principalHandle, plannerAlias),
        ...
        bool forceReconfigure)
    {
        if (squad.SquadType == SurfaceSquadType.Swarm)
        {
            yield return new AgentConfiguration
            {
                ...
                Models = "default",
                Description = $"Planner for squad {squad.Name}",
                SystemPrompt = "You are the planner for a FabrCore Surface swarm. Break user goals into concise task guidance for the available agents. Return readable Markdown.",
                Args = BuildBaseArgs(squad, runtimeJson, "planner"),
        ...
        {
            Handle = squad.OrchestratorHandle,
            AgentType = squad.SquadType switch
            {
                SurfaceSquadType.Orchestrator => SurfaceOrchestrationAgentTypes.SquadOrchestrator,
                SurfaceSquadType.Task => SurfaceSwarmAgentTypes.TaskRunner,
                _ => SurfaceSwarmAgentTypes.Orchestrator
            ...
            SystemPrompt = squad.SquadType == SurfaceSquadType.Task
                ...
                : "You are the orchestrator for a FabrCore Surface swarm. Use the available swarm agents when they can help, synthesize their replies, and keep the squad transcript readable.",
            Args = BuildBaseArgs(squad, runtimeJson, "orchestrator"),
            ForceReconfigure = forceReconfigure
        };
    }
    ...
    public static SurfaceSquad? TryReadSquad(AgentConfiguration? config)
    {
        ...
        try
        {
            ...
            else if (string.Equals(config.AgentType, SurfaceSwarmAgentTypes.TaskRunner, StringComparison.OrdinalIgnoreCase))
            {
    ...
    private static void Validate(SurfaceSquadDefinition definition)
    {
        ...
        if (definition.SquadType == SurfaceSquadType.Swarm)
        {
            throw new InvalidOperationException(
                "Swarm squads are created through ISurfaceSquadService.");
        }
    ...
    {
        var runtimeJson = SurfaceSquadRuntime.Serialize(new SurfaceSquadRuntime { Squad = squad });
        var handles = squad.SquadType == SurfaceSquadType.Orchestrator
            ? [squad.OrchestratorHandle]
            : squad.SquadType == SurfaceSquadType.Task
                ? [squad.OrchestratorHandle]
                : new[] { squad.PlannerHandle, squad.OrchestratorHandle };
        foreach (var handle in handles)
        {
            ...
            {
                Handle = handle,
                AgentType = string.Equals(handle, squad.PlannerHandle, StringComparison.OrdinalIgnoreCase)
    ...
    private static string BuildPlannerPrompt(SurfaceSquadDefinition definition)
        => string.IsNullOrWhiteSpace(definition.PlannerSystemPrompt)
            ? "You are the planner for a FabrCore Surface swarm. Break user goals into concise task guidance for the available agents. Return readable Markdown."
