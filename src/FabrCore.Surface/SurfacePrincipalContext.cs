...
public sealed class SurfacePrincipalContext : ISurfacePrincipalContext, IPrincipalGrainObserver
{
    private readonly IClusterClient clusterClient;
    private readonly ILogger<SurfacePrincipalContext> logger;
    private readonly IPrincipalGrain principalGrain;
    ...
    private SurfacePrincipalContext(
        IClusterClient clusterClient,
        ILogger<SurfacePrincipalContext> logger,
        string handle,
        IPrincipalGrain principalGrain)
    {
    ...
    internal static SurfacePrincipalContext CreateUninitialized(
        IClusterClient clusterClient,
        ILogger<SurfacePrincipalContext> logger,
        string handle,
        IPrincipalGrain principalGrain)
        => new(clusterClient, logger, handle, principalGrain);

    internal void SetObserverReference(IPrincipalGrainObserver observerReference)
    {
        if (observerRef != null)
        {
    ...
    public async Task<AgentMessage> SendAndReceiveMessage(AgentMessage request)
    {
        ...
        await RefreshObserverAsync(force: true);
        return await principalGrain.SendAndReceiveMessage(request);
    }
    ...
    public async Task SendMessage(AgentMessage request)
    {
        ...
        request.FromHandle ??= handle;
        await RefreshObserverAsync(force: true);
        await principalGrain.SendMessage(request);
    }
    ...
    public async Task SendEvent(EventMessage request)
    {
        ...
        await RefreshObserverAsync(force: true);
        await principalGrain.SendEvent(request);
    }
    ...
    public async Task<AgentHealthStatus> CreateAgent(AgentConfiguration agentConfiguration)
    {
        ...
        await RefreshObserverAsync();
        return await principalGrain.CreateAgent(agentConfiguration);
    }
    ...
    public async Task<AgentHealthStatus> ResetAgent(string handle)
    {
        ...
        await RefreshObserverAsync();
        return await principalGrain.ResetAgent(handle);
    }
    ...
    {
        ThrowIfDisposed();
        return await principalGrain.GetTrackedAgents(activate);
    }
    ...
    public async Task<bool> IsAgentTracked(string handle)
    {
        ...
        var trackedAgents = await principalGrain.GetTrackedAgents();
    ...
    {
        ThrowIfDisposed();
        return await principalGrain.GetAccessibleSharedAgents();
    }

    void IPrincipalGrainObserver.OnMessageReceived(AgentMessage message)
    {
        if (disposed)
        {
    ...
    private async Task RefreshObserverAsync(bool force = false)
    {
        ...
        if (!force && (DateTime.UtcNow - lastObserverRefresh).TotalMinutes < 3)
        {
            return;
        }
