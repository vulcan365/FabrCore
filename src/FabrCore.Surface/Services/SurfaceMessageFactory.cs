...
public static class SurfaceMessageFactory
{
        ...
        string? targetHandle = null)
    {
        ...
        message.Args = sourceMessage.Args is null
            ? new Dictionary<string, string>()
            : new Dictionary<string, string>(sourceMessage.Args);
        if (!string.IsNullOrWhiteSpace(sourceMessage.FromHandle))
        {
            message.Args[SurfaceMessageArgs.SurfaceSourceHandle] = sourceMessage.FromHandle;
        }
