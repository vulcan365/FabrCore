...
public sealed partial class SurfaceActionDispatcher : ISurfaceActionDispatcher
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var reply = new AgentMessage
        {
            ...
            FromHandle = context.PrincipalContext.Handle,
        ...
        {
            await context.PrincipalContext.SendMessage(reply);
        }
    }
    ...
    {
        if (TryGetString(payload, SurfaceActionDataKeys.ActionId, out var actionId)
    ...
    {
        if (TryGetString(payload, SurfaceActionDataKeys.RouteTo, out var routeTo))
        {
    ...
    {
        if (TryGetString(payload, SurfaceActionDataKeys.MessageTemplate, out var template))
        {
            return template;
        }
    ...
    private static string? ResolveTargetAgent(SurfaceActionContext context, Dictionary<string, object?> payload)
    {
        if (!TryGetString(payload, SurfaceActionDataKeys.TargetAgent, out var targetAgent))
        {
        ...
        var principal = HandleUtilities.ParseHandle(context.PrincipalContext.Handle).UserHandle;
        return string.IsNullOrWhiteSpace(principal)
            ? targetAgent
            : HandleUtilities.EnsurePrefix(targetAgent, HandleUtilities.BuildPrefix(principal));
    }
