...
| Phase | Contents | Size |
| --- | --- | --- |
| **P1 — Native harness core (shipped 2026-08-03, narrowed)** | Delivered: `FabrCoreHarnessAgent` assembler over core-package providers (no new package ref) + `FabrCoreHarnessOptions` + `FabrCoreHarnessResult` (run, snapshot, clear) + `AsFabrCoreHarnessAgent` / `CreateFabrCoreHarnessAgent`; **session snapshot persist/restore** (§5.2) with corruption archiving and lost-delegation reporting, **without** the grain flush (see the status note at the top); `FabrCoreBackgroundAgent` + `AgentRosterBuilder` for handle-driven delegation; loop evaluators from config; the `_Harness*` args surface (`HarnessArgs`); 21 tests in `FabrCore.Sdk.Tests`. Deferred to a later phase: modes, tool approval, in-run compaction wiring (§3.2/§3.3), provider-tool `VerifiableExecutionAIFunction` wrapping, conformance tests vs upstream `HarnessAgent`, escape-hatch docs. | done (narrowed) |
