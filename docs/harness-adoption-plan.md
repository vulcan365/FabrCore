...
> **Shipped 2026-08-03 — P1 (narrowed).** The native assembler landed in `src\FabrCore.Sdk\Harness\` covering todos, the loop, and background agents over real FabrCore handles, plus session-snapshot durability. Three deviations from the design below, each deliberate: (1) the entry points are `IChatClient.AsFabrCoreHarnessAgent(...)` (pure assembler, mirrors Microsoft's extension) and `FabrCoreAgentProxy.CreateFabrCoreHarnessAgent(...)` (ergonomic path), not the drafted `CreateHarnessAgent`; (2) **no grain change** — §5.2's per-turn custom-state flush in `AgentGrain.HandlePrimaryMessage` would have cost every agent a whole-blob write, so the harness flushes its own snapshot inside `RunAsync` and the existing deactivation flush stays the backstop; (3) tool approval, the memory socket, and provider-tool `VerifiableExecutionAIFunction` wrapping are **not** in this cut. See §9 P1.

> **Shipped 2026-08-04 — compaction ladder.** §3.2 is now implemented, with one change of scope: layer 1 is composed for **every** agent, not only harness agents. `CreateChatClientAgent` and `CreateFabrCoreHarnessAgent` both add a `CompactionProvider` with `ContextWindowCompactionStrategy` (`src\FabrCore.Sdk\ContextCompaction.cs`), fed from `ModelConfiguration.ContextWindowTokens` / `MaxOutputTokens`. Four corrections to the design below, each forced by what shipping revealed:
>
> 1. **Mid-turn destructive compaction is deleted, not "forced off under harness mode"** (§3.2, §3.3). With layer 1 bounding every call universally, there is nothing left for it to do, and keeping it as a per-config knob preserved the corruption hazard for anyone who enabled it. `MidTurnCompactionEnabled` / `_MidTurnCompactionEnabled` are accepted and ignored; `RunStopReason.MidTurnCompactionFailed` is retained but never produced.
> 2. **The `CompactionProvider` group index is stripped before the session snapshot is persisted** (`ContextCompaction.StripSessionState`). §3.1 recorded the index as "lost every activation until FabrCore persists session snapshots" — snapshots shipped on 2026-08-03, which turned that line from a limitation into a live bug: a persisted index duplicates the whole conversation into the state blob and can outlive a layer 2 rewrite, re-sending messages history compaction had already summarized away. The strategy is deterministic and LLM-free, so dropping it costs nothing.
> 3. **Threshold defaults are resolved, not hand-tuned.** History compaction defaults to 0.87 of the window when layer 1 is active and 0.75 when it is not; the projection fuse moves to the model window at 0.9. §3.2 left both as operator guidance, which would have gone wrong by default.
> 4. **The ladder is logged, once, resolved** (`CompactionLadder.Describe()`), with an out-of-order warning. Layer 2 emits `compaction.history.*` monitor events.

> **Shipped 2026-08-06 — operating modes.** The native harness now composes `AgentModeProvider` by default with FabrCore-owned `plan` / `execute` instructions. Plans live in the durable todo list, never file memory. `FabrCoreHarnessResult.RunAsync(AgentMessage)` reads `_plan-mode` on every inbound message (missing/invalid/true = plan; false = execute), while the todo evaluator only drives re-invocation in execute mode. Mode state and external SDK changes use the existing durable session snapshot.
>
> Type names are unchanged: `CompactionService` / `CompactionConfig` / `CompactionResult` still carry the layer 2 meaning, and `OnCompaction` keeps its signature, so `FabrCore.Services.Memory`'s `MemoryCompactionHandler` is unaffected. Only the user-facing vocabulary, config, and docs moved to "history compaction".
...
| **Agent modes** | ON; plan/execute; plan mode writes plan to memory + asks approval | Native harness composes durable `plan` / `execute`; `_plan-mode` selects each run; todo looping is execute-only | **SHIPPED (with instruction override)** | FabrCore's plan mode targets the durable todo list instead of rejected file memory. SDK helpers expose and persist external mode changes; channel-specific commands remain deferred |
...
**Composition (all FabrCore-chosen; nothing else exists in the pipeline):** todos + modes ON (plan-mode instructions overridden per above), tool approval ON with binding, message injection ON, per-service-call persistence ON (buffered), compaction per §3.2 from `ModelConfiguration`, **no file memory and no file access — not even as options** (decision; `fabrcore.host` storage / MCP for genuine file needs), principal-scoped Storage-backed skills only when `_HarnessSkills` or an explicit code source is supplied (never CWD discovery; MCP skills remain deferred), web search OFF (capability-gated opt-in), memory socket inert until an `IAgentMemoryService` is registered (§5.3), `MaximumIterationsPerRequest = 40`.
...
| `_HarnessSkills` (null, csv of exact `name@version`) | immutable principal-scoped skills from Host typed Storage; MCP skills remain deferred |
...
8. **Skills as managed platform content** — the Storage-backed, exact-version, per-principal source and audited admin publishing API are shipped; MCP skill composition remains follow-on work. No CWD scanning.
...
| Phase | Contents | Size |
| --- | --- | --- |
| **P1 — Native harness core (shipped 2026-08-03, expanded 2026-08-06)** | Delivered: native assembler and options/result surfaces; durable session snapshots; roster-fed background delegation; config-driven loop evaluators; the compaction ladder; and durable plan/execute modes selected per `AgentMessage`. Deferred: tool approval, provider-tool `VerifiableExecutionAIFunction` wrapping, conformance tests vs upstream `HarnessAgent`, and escape-hatch docs. | done (expanded) |
...
| **P0 — Surface Task squad (shipped 2026-08-03)** | First consumer of the harness primitives, ahead of the SDK assembler. `SurfaceTaskHarnessAgent` composes `TodoProvider` + `BackgroundAgentsProvider` through the existing `CreateChatClientAgent(configureOptions)` hook and wraps the result in a `LoopAgent` with the todo/background evaluators — no SDK change required, because `ChatClientAgentOptions.AIContextProviders` and `ChatClientAgent.GetService` already carry the composition. Replaced the 1220-line `SurfaceTaskRunnerAgent` and its JSON scraping, response envelope, `"unknown"` SME sentinel, `executors.First()` fallback, and 10ms timer trampoline. Squad members are wrapped as `SurfaceSquadMemberAgent` (not `A2AAgentProxy` — that has no `Name`/`Description`, bypasses the squad bus, and has no timeout) so delegation still mirrors into the principal timeline and now honors `DelegationTimeoutSeconds`. Single-turn by design: session snapshots (§5.2) are **not** a prerequisite because `BackgroundAgentsProvider` in-flight work is non-serializable anyway. | done |
| **P4 — Skills, modes, background agents, polish (partial)** | Shipped: principal-scoped Storage-backed `AgentSkillsSource`, exact-version `_HarnessSkills`, audited immutable ZIP publishing, durable plan/execute modes, and roster-fed background agents. Remaining: MCP skills, `A2AAgentProxy.Name` fix, reminder-driven durable-loop template, heartbeat status tracker (todo/mode → `SetStatusMessage`), and the `ChatRunSafetyScope`-aware compaction trigger (§6.9) | partial |

**Back-compat guarantees:** `CreateChatClientAgent`, `ForkAsync`/`TaskWorkingAgent`, existing compaction/projection/run-safety paths, and `IFabrCoreAgentHost` are untouched. Existing native harness agents gain default-on plan mode; set `_HarnessMode=false` to retain the prior mode-independent todo-loop behavior.
