...
`Action.Execute` and `Action.Submit` produce an `AdaptiveCardActionEvent` when routed to an agent. The event includes action type, action id, verb, route, message, target agent, payload, and optional app result.
