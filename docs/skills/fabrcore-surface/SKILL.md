...
builder.AddFabrCoreSurface(options =>
{
    ...
    // Keep the legacy manual-create compatibility surface disabled in OSS deployments.
...
- `EnableAgentCreate`: legacy manual-create compatibility flag. Keep it `false` for the OSS
  blueprint-only experience; interactive management belongs in Forge.
