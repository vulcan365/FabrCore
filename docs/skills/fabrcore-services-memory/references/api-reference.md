...
`AddMemoryAdministration()` maps `/fabrcoreapi/memory/admin/v1` and requires the
`FabrCoreAdmin` bearer policy. For a UI or service in another process:

```csharp
builder.Services.AddRemoteMemoryAdministration(options =>
{
    options.Mode = MemoryAdminClientMode.Remote;
    options.BaseAddress = "https://fabrcore-host.example.com";
    options.ApiKey = builder.Configuration["FabrCoreAdminKey"];
});
```

Resolve `IMemoryAdminClient`, not `IMemoryAdminService`, in a transport-neutral consumer.
`Auto` selects the local keyed client when available and otherwise uses
`RemoteMemoryAdminClient`. Remote calls also send the principal from
`IMemoryAdminPrincipalAccessor` as `x-user-handle`.
