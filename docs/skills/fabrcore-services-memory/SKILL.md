...
description: >
  Build, configure, test, or troubleshoot FabrCore.Services.Memory: hot/warm/cold agent
  memory, scoped shared pools, taxonomy, SQL Server 2025 vector/graph storage, retrieval,
  compaction, synthetic imagining, audit, and local or remote administration. Use for
  AddAgentMemoryServices, IAgentMemoryService, AgentMemoryPlugin, memory scopes, recall,
  extraction, MemoryAdminClientOptions, RemoteMemoryAdminClient, the mem schema, or
  /fabrcoreapi/memory/admin/v1. Use fabrcore-graphrag for knowledge graphs and
  fabrcore-agent for general agent lifecycle.
---

# FabrCore Agent Memory Service

A structured memory management library for FabrCore agents. Provides three-temperature memory (hot/warm/cold), scoped memory pools (isolated per agent by default, or shared across agents), taxonomy-enforced storage, LLM-based retrieval, freshness tracking, automatic compaction, audit logging, an administration surface, and synthetic imagining (conversation-aware multi-query recall) — with a self-contained `mem` SQL schema. The project is `FabrCore.Services.Memory`; transport contracts and shared DTOs are owned by `FabrCore.Services.Contracts`.
...
Remote administration is available at `/fabrcoreapi/memory/admin/v1`. Register
`AddRemoteMemoryAdministration()` and configure `MemoryAdminClientOptions` with
`Auto`, `Local`, or `Remote` mode, `BaseAddress`, and the cluster-scoped `ApiKey`.
`RemoteMemoryAdminClient` sends `Authorization: Bearer` for the Host's `FabrCoreAdmin`
policy and `x-user-handle` for ACL/audit attribution.
