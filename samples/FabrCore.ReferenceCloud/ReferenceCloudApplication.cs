...
internal static class ReferenceCloudApplication
{
        ...
        Func<string, string?>? readEnvironment = null)
    {
        ...
        var reports = new ConfigurationReportStore(clusterId, environment);
        ...
        app.MapPost(CloudServerProtocol.HeartbeatPath, (CloudHeartbeatRequest heartbeat) =>
        {
            try { reports.Accept(heartbeat); }
            catch (ArgumentException) { return Results.BadRequest(new { error = "Invalid heartbeat or configuration report." }); }
            return Results.Ok(new CloudHeartbeatResponse());
        });
        app.MapGet("/operator/configuration/reports", () => Results.Ok(reports.Snapshot()));
        app.MapPost("/operator/configuration/draft", async (ConfigurationDraftRequest request, HttpContext context) =>
        {
            var contents = await File.ReadAllTextAsync(configurationFile, context.RequestAborted);
            var desired = System.Text.Json.JsonSerializer.Deserialize<CloudConfigurationEnvelope>(contents, System.Text.Json.JsonSerializerOptions.Web)!;
            try
            {
                var settings = reports.CreateDraft(request.HostInstanceId, request.Keys, desired.Settings ?? []);
                // Return a reviewable draft only. Publishing must use the server's revision/concurrency policy.
                return Results.Ok(new { baseRevision = desired.ConfigurationVersion, settings });
            }
            catch (ArgumentException) { return Results.BadRequest(new { error = "The selected observations cannot be adopted." }); }
        });
        ...
        string Required(string name) => (readEnvironment ?? Environment.GetEnvironmentVariable)(name) is { Length: > 0 } value ? value : throw new InvalidOperationException(name + " is required.");
