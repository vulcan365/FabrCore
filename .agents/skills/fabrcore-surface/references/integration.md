...
The OSS command center exposes `/surface`. Provision agents and squads with canonical
`FabrCoreBlueprint` documents; base Surface does not map interactive creation routes.
...
Surface sends this canonical shape to the Host-owned blueprint resource. The apply result reports
the expanded agent configurations and squad counts.
